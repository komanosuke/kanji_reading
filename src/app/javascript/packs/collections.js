...
//6学年分のタイトル＆漢字ボタン（クローンのベースを作成）
let $gradeClone = document.getElementsByClassName('container_to_clone');
for(let i = 0; i < 5; i++){
	let cloneElement = $gradeClone[i].cloneNode(true);
	$gradeClone[i].after(cloneElement);
}
...
for(let i = 0; i < 6; i++){
	...
	for(let j = 0; j < kanjiList[i].length-1; j++){
		let kanjiCloneElement = $kanjiClone[0].cloneNode(true);
		$kanjiClone[clonePosition].after(kanjiCloneElement);
	}
	clonePosition += kanjiList[i].length;
}

//小１から小６まで書き込み
for(let i = 0; i < 6; i++){
	document.getElementsByClassName('grade')[i].textContent = '小学校' + (i+1) + '年生の漢字';
}

//漢字の書き込み
const $kanjiList = document.getElementsByClassName('cards');
let position = 0;
for(let i = 0; i < 6; i++){
	for(let j = 0; j < kanjiList[i].length; j++){
		$kanjiList[position].textContent = kanjiList[i][j];
		position++;
	}
}
...
function changeColor(){
	for(let i = 0; i < kanjiList.length; i++){
		color = colorList[i];
		for(let j = 0; j < kanjiList[i].length; j++){
			$cardList[k].style.backgroundColor = color;
			k++;
		}
	}
}
...
//hoverの後の処理
let kanjiCount = [0, 80, 80+160, 80+160+200, 80+160+200+202, 80+160+200+202+193, 80+160+200+202+193+191];

function setAfterHover(start, plus, color){
	for(let i = start; i < plus; i++){
		$cardList[i].addEventListener('mouseleave', function() {
			$cardList[i].style.backgroundColor = color;	
		});
	}
}
for(let i = 0; i < colorList.length; i++){
	setAfterHover(kanjiCount[i], kanjiCount[i+1], colorList[i]);
}
...
//forで正解配列情報からcssを変える&画像表示設定
for(let i = 0; i < correctList.length; i++){
	for(let j = 0; j < correctList[i].length; j++){
		if(correctList[i][j] === ''){
			continue;
		} else {
			let num = correctList[i][j];
			num = Number(num);
			num += kanjiCount[i];
			$cardList[num].style.backgroundColor = newColorList[i];
			$cardList[num].addEventListener('mouseleave', function() {
				$cardList[num].style.backgroundColor = newColorList[i];	
			});
